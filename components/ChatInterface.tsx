      ...
      <div ref={chatContainerRef} className="flex-1 p-4 space-y-4 overflow-y-auto scrollbar-thin">
        {chatHistory.map((chat, index) => (
          <div key={index} className={`flex items-start gap-3 ${chat.role === 'user' ? 'justify-end' : ''}`}>
            {chat.role === 'model' && <BotIcon className="h-8 w-8 flex-shrink-0 text-indigo-400 bg-slate-700 p-1.5 rounded-full" />}
            <div className={`max-w-xl p-3 rounded-xl ${
             ...
             {chat.role === 'user' && <UserIcon className="h-8 w-8 flex-shrink-0 text-slate-400 bg-slate-700 p-1.5 rounded-full" />}
          </div>
        ))}
        {isStreaming && (
           <div className="flex items-start gap-3">
             <BotIcon className="h-8 w-8 flex-shrink-0 text-indigo-400 bg-slate-700 p-1.5 rounded-full" />
      ...
      <div className="p-4 border-t border-slate-700/50">
        <form onSubmit={handleSubmit} className="relative">
          <input
            ...
            className="w-full pl-4 pr-12 py-3 bg-slate-800 border border-slate-600 rounded-full text-slate-200 placeholder-slate-500 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
            disabled={isStreaming}
          />
          <button type="submit" disabled={isStreaming || !message.trim()} className="absolute right-2 top-1/2 -translate-y-1/2 p-2 bg-indigo-600 text-white rounded-full hover:bg-indigo-500 disabled:bg-indigo-900/50 disabled:cursor-not-allowed transition-colors">
